...
namespace CheckersConsoleApp.RatingStrategies
{
    public class CirclesStrategy : IRatingStrategy
    {
        ...
        {
            return board.Pawns.Where(p => p.Side == GameSide.White).Select(RateCircle).DefaultIfEmpty(0).Sum() /
                   board.Pawns.Where(p => p.Side == GameSide.Black).Select(RateCircle).DefaultIfEmpty(1).Sum();
        }
